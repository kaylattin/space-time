...
// MAIN MODEL
  ...
  int abund[ncounts];   // Total abundance
...
// OBSERVER SUB-MODEL --------------------
  ...
  // observed data
  int abund_obs[ncounts_obs];   // Total abundance at a route
...
abund_obs ~ poisson_log(lambda_obs);
...
// MAIN MODEL 
 ...
 noise ~ normal(0, sigma_n);  // Prior for noise
  ...
  // likelihood
    ...
    lambda[i] = a[reg[i]] + b_time[reg[i]] * time[i] * pforest[i] + b_space[reg[i]] * space[i] * pforest[i] + obs_offset[obs[i]] + noise[i];
    }
    
abund ~ poisson_log(lambda);          
...
generated quantities{
  ...
  for(i in 1:ncounts){
  y_rep[i] = poisson_log_rng(a[reg[i]] + b_time[reg[i]] * time[i] * pforest[i] + b_space[reg[i]] * space[i] * pforest[i] + obs_offset[obs[i]] + noise[i]);
  }
}
