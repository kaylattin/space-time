...
generated quantities{
  ...
  vector[nspecies] a_space_mean_sp;
  vector[nspecies] a_time_mean_sp;
...
// for each region summarized across the species in that region
  ...
  for(g in 1:nreg){
    ...
    a_space_mean_sp[s] = mean(a[2, s, sp_reg_mat[s,1:nreg_s[s]]]); // uses only the species in that region to calcualte means
  }
  ...
  for(s in 1:nspecies){
    ...
    a_time_mean_sp[s] = mean(a[1, s, sp_reg_mat[s,1:nreg_s[s]]]); // uses only the species in that region to calcualte means
  }
