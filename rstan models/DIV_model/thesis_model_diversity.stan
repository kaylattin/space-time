...
diversity_obs ~ student_t(4, mu_obs, sigma_obs);
...
diversity ~ student_t(4, mu, sigma);          
...
generated quantities{
  ...
  for(i in 1:ncounts){
  y_rep[i] = student_t_rng(4, a[reg[i], spacetime[i]] + b_time[reg[i]] * time[i] * pforest[i] + b_space[reg[i]] * space[i] * pforest[i] + obs_offset[obs[i]], sigma);
  }
}